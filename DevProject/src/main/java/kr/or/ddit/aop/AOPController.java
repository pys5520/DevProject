...
// AOPController 클래스 빈 등록시 , aoPController로 할지 aOPController로 할지가 명확하지 않을 수 있어서 aopController라는 이름을 명시해줌
...
public class AOPController {
	 ...
	 * 		그러던 어느날, 성희 팀장님을 통해서 '반장님, 지금 개발중인 서비스 처리속도좀 로그로 남겨주세요!'라는 부탁을 받습니다.
	 * 		반장 정수학생은 부탁받은 요구사항을 이행하기 위해서 본인인 만들고 있는 서비스 로직에서 처리속도를 찍어볼 메소드를 개발해 처리 속도가 잘 찍히는걸 확인합니다.
	 * 		기분이 좋은 정수학생은 팀장님께 달려갑니다. '팀장님! 기능 작업 완료했습니다.!' 라고 컨펌을 받습니다.
	 * 		성희 팀장님은 아주 긍정적인 검토안을 정수학생한테 전달하면서 우리 서브스 전체에도 각 처리속도를 찍어주세요 라고 다시 부탁합니다.
	 ...
	 * 		나타날수 있고, 중복코드를 만들어 내 개발의 효율성을 저하시키고 유지보수가 힘들어 질 수 있습니다. 
	 * 
	 ...
	 * 																		분리해서 한곳으로 모으는 것을 의미
	 * 			- @Component : @Aspect와 짝꿍, component-scan시 "저 여기 있어요! 봐주세요!"라는 의미
	 * 			- JoinPoint : 어드바이스(부가기능)가 적용될 수 있는 위치
	 * 			- Advice : 어떤 부가기능(메소드 실행 시작 전, 메소드 실행 후등)을 언제(삼겹살을 굽기 전, 삼겹살으 다 먹은 후등) 사용할지 정의
	 * 				* 언제 ?
	 ...
	 * 
	 * 		- 횡단 관심사
	 * 		> 핵심 브즈니스 로직과 다소 거리가 있지만, 여러 모듈에서 공통적이고 반복적인 처리를 요구하는 내용이다.
	 * 
	 * 		- 횡단 관심사의 분리
	 ...
	 * 			- 조인포인트(JoinPoint)
	 * 				> 횡단 관심사가 실행될 지점이나 시점(메서드 실행이나 예외발생 등)을 말한다.
	 * 				> 어디에 적용할 것인지 결정, 메소드? 객체생성시? 등등
	 * 			- 어드바이스(Advice)
	 * 				> 특정 조인 포인트에서 실행되는 코드로, 횡단 관심사를 실제로 구현해서 처리하는 부분이다.
	 * 				> 어떤 부가 기능을 구현할 것인지 결정(Before, After, AfterReturning, AfterThrowing, Around)
	 * 			- 포인트컷(Pointcut)	
	 * 				> 수 많은 조인 포인트에서 실행되는 코드로, 횡단 관심사를 실제로 구현해서 처리하는 부분이다.
	 * 				> 어드바이스가 적용될 지점
	 * 			- 위빙(Weaving)
	 ...
	 * 					.. 				|	임의의 패키지 0개 이상 계층을 의미하거나 임의의 인수 0개이상을 의미한다.
