...
public class MemberController {
	...
	// 6) HTML Form 필드값이 숫자일 경우에는 컨트롤러 매개변수 타입이 문자열이면 그대로 문자열 현태로 돌아가는가?
	...
	public String register(String userId, String password, int coin) {
	...
	// 정확하게 각 필드명이 일치했을때만 파라미터를 받을수 있다.
	...
	/*
	 * 5. Date 타입 처리
	 * - 스프링 MVC는 Date 타입의 데이터를 처리하는 여러 방밥을 제공한다.
	...
	// 4) 폼 라디오버튼 요소값을 기본 데이터 탑인 문자열 타입 매개변수로 처리한다.
	...
	public String registerCheckbox03(ArrayList<String> hobbyList) {
		...
		// 리스트와 같은 형태의 값을 받으려면 String[]로 여러데이터를 받아서 List에 담는 방법이다,
		// 객체를 이용하여 데이터를 바인딩하는 방법이 잇다.
		log.info("registerCheckbox03()실행...!");
		
		if(hobbyList != null) {
			log.info("hobbyArray.length : " + hobbyList.size());
			for(int i=0; i<hobbyList.size(); i++) {
				log.info("hobbyArray[" + i+ "]: " + hobbyList.get(i));
			}
		}else {
	...
	public String registerDate01(Date dateOfBirth) {
		// 결과로 Date타입의 값을 받기 위해서는 default인 2023/05/24의 형태로 값을 설정해서 보내야한다.
	...
	// 2) 파일업로드 폼 파일 요소값과 텍스트 필드 요소값을 MultipartFile 매개변수로 처리한다.
	...
	// 5) 여러 개의 파일업로드를 폼 파일 요소값을 여러 개의 MulitpartFile 매개변수로 처리한다.
	...
	// 7) 여러 개의 파일업로드를 폼 파일 요소값을 MultipartFile 타입의 요소를 가진 자바빈즈 매개변수로 처리한다.
