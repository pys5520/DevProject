...
public class MybatisController {
	 ...
	 * 
	 * 4. '_'로 구분된 컬럼명 자동 매핑
	 * - 마이바티스 설정의 mapUnderscoreToCamelClose 프로퍼티 값을 true로 지정하면 '_'로 구분된 컬럼명을 소문자 낙타표기법의
	 * 		프로퍼티명으로 자동 매핑할 수 있다.
	 * 
	 ...
	 * 				생성된 boardNo를 sql쿼리가 당겨있는 insert 태그까지 넘어올때 넘겨주고 있는 파라미터(board)의 property인
	 * 				boardNo에 셋팅한다. 셋팅된 boardNo를 아래에서 insert 시 등록값으로 사용하고 board라는 자바빈즈 객체에 담겨
	 * 				최종 컨트롤러까지 넘어간다.
	 * 
	 ...
	 * 			- 화면 목록 화면 서비스 클래스 메소드 만들기
	 * 			- 화면 목록 화면 Mapper 인터페이스 메소드 만들기
	 * 			- 화면 목록 화면 Mapper xml 쿼리 만들기
	 * 			- 화면 목록 화면 페이지 만들기 (crud/member/list.jsp)
	 * 			- 여기까지 확인
	 * 
	 ...
	 *
	 *  - 페이지 모듈화를 위한 Tiles를 함께 사용하여 CRUD를 진행해봅시다			 
	 *  
	 *  1. Tiles란?
	 *  
	 ...
	 *   	- footer.jsp (footer 영역에서 사용할 페이지 입니다.)
	 *   	
