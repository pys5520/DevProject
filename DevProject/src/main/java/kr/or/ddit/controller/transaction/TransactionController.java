...
public class TransactionController {
	 ...
	 * 			삭제, 수정도 마찬가지로 이루어진다.
	 * 		
	 * 		@Transactional 을 적용하고 중간에 에러를 발생 시킨 후에, 롤백된 상태를 확인하려고 했으나 롤백이 되지 않고 데이터베이스에 데이터가 등록된다.
	 * 
	 ...
	 * 		실제로 '트랜잭션 롤백 처리', 'checked Exception vs unchecked Exception' 등 검색 해보면 잘못된 정보들로 복사/붙여놓고 되어 있는 내용이
	 * 		수두룩하여 많은 에러 정보를 공유하고 있다. (모든 블로그가 그렇지 않지만, 대표적인 표가 무수히 많이 공유되고 있다.)
	 * 
	 * 		***
	 * 		'예외처리 시 트랜잭션 처리'에 대한 정보가 잘못되어 표기된 경우가 많다.
	 * 		이럴때 되고 저럴때 되고가 아니라, 기본적인 스프링 트랜잭션의 정책이 있고 그 안에서 정책 이외에 옵션으로 특정 에러가 발생했을 때
	 * 		롤백을 진행할 수 있도록 제공한다. (많은 블로그들은 100% 믿지 말길 바랍니다... 혹여나 블로그를 작성한다면 검증된 내용으로만 적어주길 바랄게요..)
	 * 
	 ...
	 * 		클래스			|	IOExeption, SQLException...				|	NullPointException, IndexOutOfBoundException...
	 * 		-----------------------------------------------------------------------------------------------------------
	 * 
	 ...
	 * 				- 새롭게 시작된 트랜잭션에만 적용되므로, Propagation.REQUIRED 또는 Propagation.REQUIRES_NEW와 함꼐 사용되도록 독점 설계 되었다.
	 * 
	 * 			1-1) 옵션
	 * 		
	 * 			** 용어
	 ...
	 * 							Non-repeatable 및 Phantom Read가 발생할 수 있습니다.
	 ...
	 * 				- MANADATORY : 현재 트랜잭션을 지원하고, 없으면 예외를 발생시킨다.
	 ...
	 * 				- NEBER : 트랜잭션이 아닌 방식으로 실행하고 트랜잭션이 있으면 예외를 발생시킵니다.
	 * 					> 실행 자체가 트랜잭션을 필요로 하지 않고, 트랜잭션이 존재하다면 예외를 발생시킨다.
	 ...
	 * 			- 기본값은 false입니다.
	 * 			- readOnly 속성을 설정했다고 해서, 읽기 전용으로 무조건 설정된다는 보장이 없음. (쓰기와 같은 트랜잭션이 실행될 수 도 있음)
	 * 				> 읽기 전용에 대한 힌트를 분석할 수 없는 트랜잭션인 경우 throw를 던지지 않고 조용히 힌트를 무시합니다.
	 * 
	 * 		4) timeout (트랜잭션 제한시간)
	 * 			- 기본값은 -1로 무제한입니다.
	 * 			- timeout은 클라이언트와 서버와의 통신중, 서버측 문제로 다음 처리를 이어나가지 못하는 'Deadlock'을 방지할 수 있는 속성입니다.
	 * 			- 클라이언트와 서버간의 Restful API를 개발시 고려해볼 속성
	 * 
	 * 		5) rollbackFor
	 * 			- 트랜잭션 롤백을 유발해야 하는 예외 유형을 나타내는 0개이상의 예외 유형을 정의합니다.
