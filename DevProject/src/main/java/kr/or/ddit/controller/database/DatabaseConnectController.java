...
public class DatabaseConnectController {
	 ...
	 *  
	 *  5. 스프링 JDBC 
	 *   - SQL로만 데이터베이스를 쉽게 처리할수 있도록 도와주는 JDBCTemplate 클래스를 제공한다.
	 *   
	 ...
	 *   	3. 게시판 구현
	 *   
	 *   		기본적인 JDBC를 이용한 디비 설계는 DAO란에서 sql쿼리가 노출되어 개발하는 방식이다보니 우리가 중급프로젝트때 또는 JSP에서
	 ...
	 *   		그리고, 사용하는 데이터베이스 종류가 변경된다 하더라도 문제가 없다. (JPA는 SQL을 직접 건들이지않고 함수와 같은 기능들로만 쿼리를 요청하고 
	 *   		데이터베이스와 통신하기 때문이다.)
	 *   
	 ...
	 *   		- 예제로 mapper패키지안에 BoardMapper.xml을 구성하여 테스트를 진행합니다!
	 *  
	 */
}
