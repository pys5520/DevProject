...
public class BoardController {
	 ...
	 *  - 클래스 레벨로 요청 경로를 지정하면 메소드 레벨에서 지정한 경로의 기본 경로로 취급한다.
	 ...
	 * 화면으로 응답하는 경우에는 HTTP메서드로 GET방식과 POST방식 두가지를 사용할 수 있다.
	 */
	 ...
	 * remove() { logger.info("remover() 실행...!"); return "success"; }
	 * 
	 * // list 경로에 GET 방식
	 * 
	...
	// 응답 상태코드와 값을 제네릭에 설정되어 있는 타입에 따라서 내 마음대로 내보낼수 있다.
	...
	/*
	 * 7. Accept 매핑 사용 
	 * - produces 속성값을 지정해서 요청하는 미디어 타입을 처리한다. (미디어타입으로 나갈때 쓰임)
	 */
	...
	// produces 속성값에 "applicaton/json" 미디어 타입을 지정한다.
	...
	// produces 속성값에 "applicaton/xml" 미디어 타입을 지정한다.
