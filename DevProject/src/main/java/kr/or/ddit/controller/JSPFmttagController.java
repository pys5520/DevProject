...
public class JSPFmttagController {
	 ...
	 * 		var			|	String			|	포맷팅한 결과를 지정할 변수 이름
 	 * 			
 	 *		** type 속성 : number일 경우 숫자 형식으로, percent일 경우 % 형식으로, currency일 경우 통화형식으로 출력한다.
 	 *			기본값은 number이다. currency는 속해있는 국가코드에 맞는 통화형식이 부여된다.
 	 *
 	 * 		2) <fmt:parseNumber>
 	 * 		- 문자열을 숫자로 변환한다.
 	 * 
	 ...
	 * 		type		|	String			|	날짜, 시간 또는 둘다 포맷팅할지의 여부를 지정한다.
	...
	// 만약 type 속성이 percent인 경우에 넘겨받아야할 값이 '1000%'와 같이 %의 스타일 형태 값이 넘어가야한다.
	...
	// 8) type 속성을 both로 지정하여 날짜,시간 둘다 포맷팅을 한다.
...
//		String dateValue = "2020년 2월 1일(금)";	// style long 형태로 지정
//		String dateValue = "2020년 2월 1일 금요일";	// style full 형태로 지정
		// 각 dateStyle로 지정된 값이 페이지로 넘어가 pasrsing이 진행될때, parseDate내에 
	...
	public String home0403(Model model) {
		String dataValue = "2022-02-01 15:00:24";
