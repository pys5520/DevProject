...
<body>
	...
	<p>1) URL 경로 상의 쿼리 파라미터 정보로부터 요청 데이터를 취득할 수 없다.</p>
	<a href="/register?userId=hongkd&password=1234">BUTTON1</a><br/>
	...
	<p>6) HTML Form 필드값이 숫자일 경우에는 컨트롤러 매개변수 타입이 문자열이면 그대로 문자열 현태로 돌아가는가?</p>
	...
	<p>5) Member 매개변수와 쿼리 파라미터(dateOfBirth=20180908)로 전달받은 값이 날짜 문자열 형식으로 설정시 ,Date 타입으로 받는가?</p>
	...
	<p>4) 폼 라디오버튼 요소값을 기본 데이터 탑인 문자열 타입 매개변수로 처리한다.</p>
	...
	<p>13) 폼 체크박스 요소값을 기본데이터 타입인 불린 타입 매개변수로 처리한다.</p>
	...
	<form action="/registerAddress" method="post">
		poctCode : <input type="text" name="postCode"><br/>
	...
	<form action="/registerUserAddress" method="post">
		poctCode : <input type="text" name="address.postCode"><br/>
	...
	<p> 2) 파일업로드 폼 파일 요소값과 텍스트 필드 요소값을 MultipartFile 매개변수로 처리한다.</p>
	...
	<p>5) 여러 개의 파일업로드를 폼 파일 요소값을 여러 개의 MulitpartFile 매개변수로 처리한다.</p>
	...
	<p>7) 여러 개의 파일업로드를 폼 파일 요소값을 MultipartFile 타입의 요소를 가진 자바빈즈 매개변수로 처리한다.</p>
