...
// 		let content = $('#boContent').val();
		let content = CKEDITOR.instances.boContent.getData();	// 내용 데이터 얻어오기
		
		// 제목을 입력하지 않았을때에 대한 필터
		if(title == ""){
			alert("제목을 입력해주세요!");
			$('#boTitle').focus();
			return false;
		...
		if($(this).val() == '수정'){
			$('#boardForm').attr("action", "/board/update.do");		// 수정을 하러가기 위해 url경로를 틀어준다.
		}
